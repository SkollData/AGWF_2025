...
function renderDistrictsCarousel(data) {
    ...
    data.forEach(dist => {
        ...
        card.className = "flex-none w-72 md:w-80 glass-panel rounded-2xl p-6 flex flex-col items-center justify-center gap-4 snap-center border border-white/40 shadow-xl group hover:shadow-2xl transition-all bg-white/95";
        ...
        if (munis.length > 0) {
            ...
            footerHtml = `
                <div class="mt-2 w-full flex flex-col gap-3">
                    <a href="${dist.url}" target="_blank" class="w-full text-center px-4 py-2 rounded-xl bg-gradient-to-r from-primary to-primary-dark text-white font-bold text-sm shadow-md hover:shadow-lg hover:scale-[1.02] transition-all flex items-center justify-center gap-2">
                        Explorar Distrito <span class="material-symbols-outlined text-sm">arrow_forward</span>
                    </a>
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-gray-400 text-sm pointer-events-none">location_city</span>
                        <select onchange="if(this.value !== '#') window.open(this.value, '_blank')" class="w-full text-sm bg-gray-50 border border-gray-200 text-text-sub rounded-xl pl-9 pr-8 py-2 outline-none focus:ring-2 focus:ring-primary/20 cursor-pointer appearance-none">
                            <option value="#" selected disabled>Ver Municipios...</option>
                            ${options}
                        </select>
                        <span class="absolute right-3 top-1/2 -translate-y-1/2 material-symbols-outlined text-gray-400 text-sm pointer-events-none">expand_more</span>
                    </div>
                </div>
            `;
        } else {
            footerHtml = `
                <a href="${dist.url}" target="_blank" class="mt-4 px-6 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primary-dark text-white font-bold text-sm shadow-lg shadow-primary/30 hover:shadow-primary/50 hover:scale-105 transition-all flex items-center gap-2">
        ...
        card.innerHTML = `
            <div class="size-16 rounded-full bg-gradient-to-tr from-primary to-primary-dark flex items-center justify-center text-white font-black text-2xl shadow-lg">
                ${dist.id}
            </div>
            <div class="text-center">
                <h3 class="text-xl font-bold text-text-main dark:text-white">Distrito Local ${dist.id}</h3>
            </div>
            ${footerHtml}
        `;
